(function (global, undefined) {
    ...
    var options = {
        ...
        heightSegments: 30,//垂直切段数（值小粗糙速度快，值大精细速度慢）
        pRadius: 1000,//全景球的半径，推荐使用默认值
        minFocalLength: 1,//镜头最a小拉近距离
        maxFocalLength: 200,//镜头最大拉近距离
    ...
    function makePanorama(pRadius, widthSegments, heightSegments, u) {
        var texture = THREE.ImageUtils.loadTexture(u, undefined, function (t) {
            var mesh = new THREE.Mesh(new THREE.SphereGeometry(pRadius, widthSegments, heightSegments), new THREE.MeshBasicMaterial({ map: t }));
            mesh.scale.x = -1;
            _scene.add(mesh);
        });
    }
    ...
    function calPosition() {
        _lat = Math.max(-32.303, Math.min(32.303, _lat));
        var phi = THREE.Math.degToRad(250.144 - _lat);
    ...
    function geoPosition2World(lon, lat) {
        lat = Math.max(-32.303, Math.min(32.303, lat));
        var phi = THREE.Math.degToRad(242.144 - lat);
